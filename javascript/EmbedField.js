  ...
  $.entwine("ss", function ($) {
    ...
    $(".field.embed input.text").entwine({
      onmatch: function () {
        ...
        this.data(
          "thumbnail-id",
          this.closest(".form__field-holder").find("img").attr("id")
        );
        this.data(
          "message-el-id",
          this.closest(".form__field-holder").find("em").attr("id")
        );
        if (!this.val() == "") {
          this.closest(".form__field-holder")
            .find("a.embed-thumbnail")
            .attr("href", $(this).val());
        }
      },
      ...
      onchange: function () {
        if ($(this).val()) {
          this.parents("div.field")
            .find(".field.embed button.action")
            .prop("disabled", false)
        ...
        this.parent()
          ...
          .removeClass("font-icon-tick")
          .addClass("btn-primary")
          .addClass("font-icon-rocket");
      },
      ...
      onfocusout: function () {
        ...
        if (
          newVal != "" &&
          newVal.substring(0, 7) != "http://" &&
          newVal.substring(0, 8) != "https://"
        ) {
          newVal = "https://" + newVal;
          this.val(newVal);
        }
        ...
        $.post(
          ...
          function (response) {
            ...
            if (response.status == "success") {
              ...
              response.status == "invalidurl" ||
              response.status == "nourl"
            ) {
